...
아래 답변은 외워서 읽기보다 **결론 → 구현 근거 → 이유** 순서로 설명하기
위한 말하기 예시입니다. 평가자에게는 먼저 굵은 문장의 요지를 답하고,
추가 질문이 있을 때 뒤의 파일·메서드와 설계 이유를 설명합니다.
...
**네. 정답 판정은 `Quiz.is_correct()`가 선택 번호와 `answer`를 비교하고,
...
**네. 원격 저장소는 `vivleon/ia-codyssey-2`이고, 2026-08-12 증빙 기준 원격
`main`에 의미 있는 커밋 20개가 있습니다.** 현재 로컬 후속 보완까지 포함하면
21개이며, 평가 최소 기준인 10개를 넘습니다. 원격 URL·커밋 해시·커밋 수는
`docs/git-evidence.txt`에 기록되어 있습니다.
...
**네. `codex/quiz-play` 브랜치의 두 커밋이 `9a0d68d` 병합 커밋을 통해
`main`으로 합쳐진 기록이 남아 있습니다.** `--no-ff` 병합을 사용했기 때문에
기능 브랜치의 분기와 병합 지점이 그래프에 유지됩니다.
...
**네. README에 clone 명령과 실습 절차가 있고, `272f5fe` 커밋은 별도
복제본에서 만든 변경을 원격에 올린 뒤 기존 폴더에서 pull한 실습 기록입니다.**
실행 화면과 Git 그래프는 `docs/screenshots/`에서 확인할 수 있습니다.
`clone`은 원격 저장소 전체를 새 폴더로 복제하고, `pull`은 이미 존재하는
로컬 저장소에 원격의 새 변경을 가져와 통합합니다.
...
**변경 이유와 책임이 서로 다른 코드는 별도 메서드로 분리했습니다.** 입력
형식은 `_read_int()`·`_read_non_empty()`, 게임 규칙의 진행은
`play_quiz()`, 개별 정답 판정은 `Quiz.is_correct()`, 파일 영속성은
`load_state()`·`save_state()`가 맡습니다. 따라서 문구나 저장 방식이 바뀌어도
정답 판정 코드를 함께 고칠 필요가 없습니다.
...
**`run()`이 `KeyboardInterrupt`와 `EOFError`를 잡아 종료 안내를 출력하고,
어떤 종료 경로에서도 `finally`의 `save_state()`가 실행되게 했습니다.** 현재는
계속 열린 네트워크나 데이터베이스 자원이 없으므로 상태 저장이 핵심 정리
작업입니다. 미래에 외부 자원이 생기면 같은 `finally`에서 연결도 닫아야 합니다.

#### 12) 커밋 단위와 메시지 규칙은 무엇인가?

**메뉴·모델·데이터·출제·저장·조회·테스트·문서처럼 하나의 목적 단위로
커밋했습니다.** 메시지는 `Type(Scope): 변경 요약` 또는 scope를 생략한
`Type: 변경 요약` 형식을 사용합니다. `Feat`, `Fix`, `Test`, `Docs`,
`Refactor`, `Data`, `Chore`로 변경 성격을 표시하며, 커밋 전에는
`git diff --cached`로 한 가지 목적만 포함됐는지 확인합니다.

### 항목 3. 핵심 기술 원리 적용

#### 13) 클래스를 사용한 이유와 함수만 사용할 때의 차이는 무엇인가?

**퀴즈 목록, 최고 점수, 파일 경로처럼 여러 동작이 공유하는 상태와 그 동작을
함께 관리하기 위해 클래스를 사용했습니다.** 함수만 사용하면 이 상태를 여러
함수의 인자와 반환값으로 계속 전달해야 합니다. 반면 상태가 없는 작은 계산은
함수가 더 단순합니다. 그래서 개별 문제는 `Quiz`, 게임 생명주기는
`QuizGame`, 기본 데이터 생성처럼 상태가 없는 작업은 함수로 구성했습니다.

#### 14) JSON으로 저장하는 이유와 JSON 형식의 특징은 무엇인가?

**JSON은 사람이 읽기 쉽고 가벼우며 Python 표준 라이브러리만으로 처리할 수
있어 작은 학습용 데이터 저장에 적합합니다.** 이름과 값의 쌍으로 된 객체는
Python `dict`, 배열은 `list`, 문자열과 숫자는 `str`·`int`에 자연스럽게
대응합니다. UTF-8로 저장하면 한글도 보존됩니다. 단, 인덱스·트랜잭션·부분
수정 기능이 없어 대규모나 동시 쓰기에는 데이터베이스가 더 적합합니다.

#### 15) 파일 입출력에서 `try/except`가 필요한 이유는 무엇인가?

**파일은 코드 밖의 환경에 의존하므로 정상 입력만 가정할 수 없기 때문입니다.**
파일 부재, 권한 부족, 디스크 부족, 잘못된 JSON 문법, 예상과 다른 데이터
타입이 발생할 수 있습니다. `load_state()`는 `OSError`,
`JSONDecodeError`, `TypeError`, `ValueError`를 처리해 백업·복구하고,
`save_state()`는 `OSError`를 처리해 대상과 임시 파일 경로를 안내합니다.
예상 가능한 오류만 구체적으로 잡아 프로그래밍 오류까지 숨기지 않습니다.

#### 16) 브랜치를 분리한 이유와 merge의 의미는 무엇인가?

**안정적인 `main`을 유지하면서 기능을 독립적으로 개발하고 테스트하기 위해
브랜치를 분리했습니다.** merge는 검증이 끝난 브랜치의 변경 이력을 대상
브랜치에 통합하는 작업입니다. 이 프로젝트는 `--no-ff`를 사용해 단순히
커밋을 일렬로 붙이지 않고 기능 개발의 시작과 병합 지점을 기록으로 남겼습니다.
...
**전체 파일을 매번 메모리에 읽고 전체를 다시 저장하므로 데이터가 늘수록
시작 시간·저장 시간·메모리가 함께 증가합니다.** 검색도 인덱스 없이 O(N)이고,
동시 저장 시 마지막 쓰기가 앞선 변경을 덮을 수 있습니다. 1,000개 자체는
현재 측정에서 동작 가능했지만, 지속적으로 커지면 고유 ID·카테고리 인덱스와
페이지네이션을 먼저 적용하고 부분 수정·동시성이 필요할 때 SQLite로
전환합니다. 재현 수치는 `docs/state-benchmark.md`에 있습니다.

#### 19) `state.json`이 손상되면 데이터 손실을 줄이기 위해 어떻게 대응하는가?

**현재 구현은 손상 파일을 바로 덮지 않고 `state.json.broken-날짜.bak`으로
복사한 뒤 기본 데이터로 정상 JSON을 다시 만들며, 최근 백업 3개를 보관합니다.**
이렇게 하면 프로그램은 계속 실행되고 원본 손상 내용도 수동 복구를 위해
남습니다. 복구 후에는 JSON 유효성, 기본 퀴즈 수, 재실행 로드와 테스트를
확인합니다. 다만 손상 파일만으로 모든 사용자 데이터를 자동 복원할 수는
없으므로 중요한 운영 데이터라면 정상 상태의 정기 외부 백업과 스키마 버전
관리가 추가로 필요합니다.

#### 20) 채점 방식이나 선택지 개수가 바뀌면 어디를 먼저 수정하는가?

**먼저 데이터 불변 조건과 핵심 규칙을 수정하고, 그다음 입력·진행·저장·테스트
순서로 영향 범위를 확인합니다.** 정답 판정 규칙은 `quiz.py`의
`is_correct()`, 점수 계산과 최고점 비교는 `quiz_game.py`의
`play_quiz()`·`_is_new_best()`를 먼저 봅니다. 선택지 수는
`Quiz.__post_init__()`의 검증, `add_quiz()`의 입력 반복, `play_quiz()`의
입력 범위, JSON 스키마와 관련 테스트를 함께 수정해야 합니다. 변경 전후 전체
테스트를 실행해 기존 저장 데이터와의 호환성도 확인합니다.
